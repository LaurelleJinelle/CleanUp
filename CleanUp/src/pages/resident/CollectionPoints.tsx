          ...
          <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
            ...
            <div className="space-y-2 max-h-[500px] overflow-y-auto">
              {filteredPoints.map((point) => (
                <div
                  key={point.id}
                  className={`p-3 rounded-lg cursor-pointer transition-colors ${selectedPoint?.id === point.id ? "bg-blue-100 border border-blue-200" : "hover:bg-gray-50 border border-gray-100"}`}
                  onClick={() => handlePointClick(point)}
                >
                  <div className="flex items-start">
                    <MapPinIcon
                      className={`h-5 w-5 mt-0.5 ${selectedPoint?.id === point.id ? "text-blue-600" : "text-gray-400"}`}
                    />
                    <div className="ml-2">
