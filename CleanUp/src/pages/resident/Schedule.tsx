...
const Schedule = () => {
  ...
  const renderCalendar = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          key={day}
          className={`h-24 border border-gray-200 p-2 ${hasEvents ? "bg-blue-50" : "bg-white"}`}
        >
          ...
          {hasEvents && (
            <div className="mt-1 overflow-y-auto max-h-16">
              {hasEvents.map((event) => (
                <div key={event.id} className="flex items-center text-xs py-1">
                  {event.icon}
  ...
  const upcomingCollections = Object.entries(scheduleData)
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
        ...
        <div className="divide-y divide-gray-200">
          {upcomingCollections.length > 0 ? (
            upcomingCollections.map(({ date, events }) => (
