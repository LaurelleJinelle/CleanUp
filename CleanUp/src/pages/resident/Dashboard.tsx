...
const ResidentDashboard = ({ user, onLogout }) => {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      <div
        className={`${isMobileMenuOpen ? "block" : "hidden"} fixed inset-0 z-20 bg-gray-600 opacity-50 transition-opacity lg:hidden`}
        onClick={() => setIsMobileMenuOpen(false)}
      />
      <div
        className={`${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"} fixed inset-y-0 left-0 z-30 w-64 transform transition duration-300 ease-in-out lg:static lg:translate-x-0`}
      >
